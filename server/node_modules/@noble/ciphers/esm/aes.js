...
const invSbox =  sbox.map((_, j) => sbox.indexOf(j));
...
const tableDecoding =  genTtable(invSbox, (s) => (mul(s, 11) << 24) | (mul(s, 13) << 16) | (mul(s, 9) << 8) | mul(s, 14));
const xPowers =  (() => {
...
export const ctr =  wrapCipher({ blockSize: 16, nonceLength: 16 }, function aesctr(key, nonce) {
...
export const ecb =  wrapCipher({ blockSize: 16 }, function aesecb(key, opts = {}) {
...
export const cbc =  wrapCipher({ blockSize: 16, nonceLength: 16 }, function aescbc(key, iv, opts = {}) {
    const pcks5 = !opts.disablePadding;
    return {
        encrypt(plaintext, dst) {
            const xk = expandKeyLE(key);
...
export const cfb =  wrapCipher({ blockSize: 16, nonceLength: 16 }, function aescfb(key, iv) {
...
export const gcm =  wrapCipher({ blockSize: 16, nonceLength: 12, tagLength: 16, varSizeNonce: true }, function aesgcm(key, nonce, AAD) {
    if (nonce.length < 8)
...
export const gcmsiv =  wrapCipher({ blockSize: 16, nonceLength: 12, tagLength: 16, varSizeNonce: true }, function aessiv(key, nonce, AAD) {
    const tagLength = 16;
...
const AESKW_IV =  new Uint8Array(8).fill(0xa6); 
